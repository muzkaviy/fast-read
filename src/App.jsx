import React, { useState, useEffect } from "react";

const defaultText =
  "O’nun Adıyla Başlarken, Hamd, âlemlerin Rabbi Allâhü Teâlâ içindir. Salât ve selâm da Peygamberimiz Efendimiz Hazreti Muhammed’edir.";

const ReadingApp = () => {
  const [text, setText] = useState(defaultText);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [windowSize, setWindowSize] = useState(6);
  const [delay, setDelay] = useState(1000);
  const [isPaused, setIsPaused] = useState(false);
  const [readingMode, setReadingMode] = useState("standart");
  const [theme, setTheme] = useState("light");
  const [wordCount, setWordCount] = useState(1); // İlk kelimeyi saymaya 1'den başla
  const words = text.split(" ");

  useEffect(() => {
    if (isPaused) return;

    if (currentIndex >= words.length - 1) return;

    const interval = setTimeout(
      () => {
        setCurrentIndex((prevIndex) => {
          setWordCount((prevCount) => prevCount + 1);
          return prevIndex + 1;
        });
      },
      currentIndex === 0 ? 1000 : getWordDelay(words[currentIndex])
    ); // İlk kelimeyi hemen göster

    return () => clearTimeout(interval);
  }, [currentIndex, words, isPaused, readingMode]);

  const getVisibleText = () => {
    const visibleWords = words.slice(currentIndex, currentIndex + windowSize);

    while (visibleWords.length < windowSize) {
      visibleWords.push("\u00A0");
    }

    return visibleWords.join(" ");
  };

  const getWordDelay = (word) => {
    if (readingMode === "hızlı") return delay * 0.7;
    if (readingMode === "derinlemesine") return delay * 1.5;
    if (/[.!?]/.test(word)) return delay * 2;
    if (/[,;:]/.test(word)) return delay * 1.5;
    return delay;
  };

  const handleTextUpload = (event) => {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = (e) => setText(e.target.result);
      reader.readAsText(file);
    }
  };

  return (
    <div
      style={{
        textAlign: "center",
        padding: "20px",
        backgroundColor: theme === "dark" ? "#333" : "#eee",
        color: theme === "dark" ? "#333" : "#111",
      }}
    >
      <h2>Hızlı Okuma Uygulaması</h2>
      <div
        style={{
          fontSize: "24px",
          fontWeight: "bold",
          padding: "10px",
          backgroundColor: "yellow",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
          width: "100%",
          maxWidth: "600px",
          minWidth: "600px", // Sabit genişlik sağlandı
          minHeight: "50px",
          whiteSpace: "nowrap",
          overflow: "hidden",
          textAlign: "center",
          boxSizing: "border-box",
        }}
      >
        {getVisibleText()}
      </div>

      {/* <div style={{ marginTop: "20px" }}>
        <label>Pencere Genişliği: </label>
        <input
          type="number"
          value={windowSize}
          onChange={(e) => setWindowSize(Number(e.target.value))}
        />
      </div> */}
      <div style={{ marginTop: "20px" }}>
        <label>Duraklama Süresi (ms): </label>
        <input
          type="number"
          value={delay}
          onChange={(e) => setDelay(Number(e.target.value))}
        />
      </div>
      <div style={{ marginTop: "20px" }}>
        <button
          style={{ marginRight: "5px" }}
          onClick={() => setDelay((prev) => Math.max(prev - 200, 200))}
        >
          Hızı Artır
        </button>
        <button onClick={() => setDelay((prev) => prev + 200)}>
          Hızı Azalt
        </button>
      </div>
      <div>
        <div style={{ marginTop: "20px" }}>
          <button
            style={{ marginRight: "5px" }}
            onClick={() => {
              setIsPaused(true); // Geçici olarak durdur
              setCurrentIndex(0); // İlk kelimeye dön
              setWordCount(0); // İstatistikleri sıfırla
              setIsPaused(false); // Hemen başlat
            }}
          >
            Baştan Başla
          </button>

          <button onClick={() => setIsPaused((prev) => !prev)}>
            {isPaused ? "Devam Et" : "Durdur"}
          </button>
        </div>
      </div>

      <div style={{ marginTop: "20px" }}>
        <label>Okuma Modu: </label>
        <select
          onChange={(e) => setReadingMode(e.target.value)}
          value={readingMode}
        >
          <option value="standart">Standart</option>
          <option value="hızlı">Hızlı</option>
          <option value="derinlemesine">Derinlemesine</option>
        </select>
      </div>
      <div style={{ marginTop: "20px" }}>
        <button
          style={{ marginRight: "5px" }}
          onClick={() => setCurrentIndex((prev) => Math.max(prev - 5, 0))}
        >
          ⏪ Geri
        </button>
        <button
          onClick={() =>
            setCurrentIndex((prev) => Math.min(prev + 5, words.length - 1))
          }
        >
          ⏩ İleri
        </button>
      </div>
      <div style={{ marginTop: "20px" }}>
        <button onClick={() => setTheme(theme === "dark" ? "light" : "dark")}>
          {theme === "dark" ? "Açık Tema" : "Karanlık Tema"}
        </button>
      </div>
      <div style={{ marginTop: "20px" }}>
        <h3>İstatistikler</h3>
        <p>Okunan Kelime Sayısı: {wordCount}</p>
      </div>
      <div style={{ marginTop: "20px" }}>
        <input type="file" accept=".txt" onChange={handleTextUpload} />
      </div>
    </div>
  );
};

export default ReadingApp;
